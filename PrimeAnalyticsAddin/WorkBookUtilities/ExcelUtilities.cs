...
namespace PrimeAnalyticsAddin.WorkBookUtilities
{
    class ExcelUtilities
    {
        ...
        public static DataTable getData (string sheetName)
        {
            ...
            Excel.Range rng = (Excel.Range)workSheet.Range[workSheet.Cells[y, 1], workSheet.Cells[y, columnNames.Count]];

            //valueRows = (string)(rng.Cells[1, 1] as Excel.Range).Value2;
            valueRows = (rng.Cells[1, 1] as Excel.Range).Value2;
            ...
            while (valueRows != null)
            {
                ...
                foreach (dynamic item in columnNames)
                {
                    ...
                    valueRows = (rng.Cells[1, x] as Excel.Range).Value2;
                    row[item.Value] = valueRows;
                    x++;
                    //MessageBox.Show(item.Value+ ":" + valueRows.ToString());
                ...
                rng = (Excel.Range)workSheet.Range[workSheet.Cells[y, 1],workSheet.Cells[y, columnNames.Count]];
                //valueRows = (string)(rng.Cells[1, 1] as Excel.Range).Value2;
                valueRows = (rng.Cells[1, 1] as Excel.Range).Value2;
            }
        ...
        public void insertDataTableToSheet (DataTable table, string workBookName)
        {
            ...
            sl.ImportDataTable(iStartRowIndex, iStartColumnIndex, table, true);


            int iEndRowIndex = iStartRowIndex + table.Rows.Count + 1 - 1;
            // - 1 because it's a counting thing, because the start column is counted.
            int iEndColumnIndex = iStartColumnIndex + table.Columns.Count - 1;


            SLTable sheetTabel = sl.CreateTable(iStartRowIndex, iStartColumnIndex, iEndRowIndex, iEndColumnIndex);
            sheetTabel.SetTableStyle(SLTableStyleTypeValues.Light9);
            sheetTabel.HasTotalRow = true;
            sl.InsertTable(sheetTabel);

            sl.SaveAs(workBookName);
